...
Finally, mitigation strategies are investigated to determine whether the
degradation in forecasting accuracy and uncertainty reliability can be reduced
when forecasting conditions become more challenging.
...
\begin{enumerate}
    ...
    A post-hoc Conformalized Quantile Regression calibration procedure and a
    congestion-aware training strategy are evaluated as complementary mitigation
    approaches. The former directly targets predictive-interval calibration,
    whereas the latter modifies the training objective to assign greater
    importance to rare high-congestion targets.
\end{enumerate}
...
Only powers of the row-normalized propagation matrix $\mathbf{P}$ are
considered, with a separate learnable transformation $\mathbf{W}_k$ for
each propagation depth.
...
subset sizes are identical, their composition differs across horizons because
the congestion condition is evaluated independently at the target timestamp
associated with each forecast horizon. Consequently, the same input window
may belong to different traffic-condition subsets at 15, 30, and 60 minutes.
...
indicates residual overconfidence under distribution shift.
This motivates the mitigation strategies investigated in
Section~\ref{sec:mitigation}, which address the observed degradation through
both post-hoc calibration and congestion-aware training.
...
coverage, particularly at the 60-minute forecasting horizon.
This section investigates two complementary mitigation strategies. First,
a post-hoc calibration procedure is applied to improve the reliability of
the predictive intervals without retraining the forecasting model. Second,
a congestion-aware training objective is evaluated to explicitly increase
the importance of high-congestion targets during model optimization.
...
Intuitively, $\hat{s}$ represents the conformal correction applied to the
original interval bounds to achieve the target coverage level on the
calibration data.
...
from 78.72\% to 79.89\% at 15 minutes and from 77.87\% to 79.07\% at
...
As a second mitigation strategy, the training objective was therefore
modified to explicitly increase the importance of high-congestion targets.
Previous work on traffic speed forecasting has addressed the imbalance
between free-flow and congested conditions through congestion-adaptive loss
weighting, assigning greater importance to prediction errors occurring under
congested traffic states \citep{zhang2025casaformer}.

Following this principle, a congestion-aware version of the masked Pinball
Loss was implemented. The congestion condition was defined using the same
training-derived criterion introduced in
Section~\ref{sec:distribution_shift}. For each target timestamp, the network
congestion ratio $C_{t+h}$ was computed from the fraction of valid sensors
whose speed was below their sensor-specific congestion threshold. A target
timestamp was classified as high congestion when

\begin{equation}
    C_{t+h} \geq \tau_{\mathrm{peak}},
\end{equation}

where $\tau_{\mathrm{peak}}$ is the network-level congestion threshold
estimated exclusively from the training data.

A congestion-dependent weight was then assigned to each forecast step as
...
\begin{equation}
    ...
    \mathbb{I}
    \left(
        C_{t_b+h} \geq \tau_{\mathrm{peak}}
    \right),
    \label{eq:congestion_weight}
\end{equation}

where $b$ denotes the sample, $h$ the forecast step, and $\lambda$ controls
the additional importance assigned to high-congestion targets. In the
implemented configuration, $\lambda=1$ was used, so that normal targets
received weight 1 and high-congestion targets received weight 2.

The resulting congestion-aware training objective is
...
\begin{equation}
    ...
    \label{eq:congestion_aware_loss}
\end{equation}

where $m_{b,h,n}$ is the target-availability mask,
$\mathcal{Q}=\{0.1,0.5,0.9\}$ is the set of predicted quantiles, and
$\rho_q(\cdot)$ denotes the Pinball Loss for quantile $q$. The congestion
weight is shared across sensors and quantiles for a given sample and forecast
step, while missing target values remain excluded through the availability
mask.

The weighting was applied independently to all 12 forecast steps used during
training. The same congestion-aware objective was also used on the validation
set for early stopping. The Graph-Temporal architecture was left unchanged,
so the mitigation modifies only the optimization objective and does not add
model parameters. With $\lambda=1$, the best validation checkpoint was
obtained at epoch 75.

Unlike CQR, congestion-aware training directly modifies the forecasting model
by emphasizing the traffic regime in which the largest performance
degradation was observed. However, the congestion information is required
only during training and validation to construct the loss weights. At
inference time, the trained model uses the same historical traffic inputs and
graph structure as the original Graph-Temporal model and does not require
knowledge of future congestion conditions. The effect of this training-time
mitigation is evaluated in the following section.


\subsubsection{Congestion-Aware Training Results}

\begin{table}[H]
\centering
\caption{Performance of the original and congestion-aware Graph-Temporal
models under high-congestion conditions.}
\label{tab:congestion_aware_results}
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{2}{c}{15 min}
& \multicolumn{2}{c}{30 min}
& \multicolumn{2}{c}{60 min} \\
\cmidrule(lr){2-3}
\cmidrule(lr){4-5}
\cmidrule(lr){6-7}
Metric & Original & CA & Original & CA & Original & CA \\
\midrule
MAE
& 4.463 & 4.410
& 5.636 & 5.483
...
Table~\ref{tab:congestion_aware_results} directly compares the original
Graph-Temporal model with the congestion-aware variant in the regime targeted
by the reweighting strategy. Under high congestion, MAE decreases from
4.463 to 4.410 at 15 minutes, from 5.636 to 5.483 at 30 minutes, and from
7.414 to 7.012 at 60 minutes. The relative improvement becomes more
pronounced as the forecasting horizon increases, reaching approximately
5.4\% at 60 minutes. A similar trend is observed for RMSE, with the largest
reduction occurring at the 60-minute horizon, from 12.296 to 11.883.

The probabilistic forecasts also improve in the targeted regime. Empirical
coverage increases from 78.72\% to 79.39\% at 15 minutes, from 77.87\% to
78.34\% at 30 minutes, and from 75.81\% to 76.98\% at 60 minutes. The
mitigation therefore reduces the undercoverage observed in the original
Graph-Temporal model, although it does not completely restore the nominal
80\% coverage at the longest forecasting horizon.

Importantly, the improvement in coverage is not obtained by simply widening
the prediction intervals. Under high congestion, interval width decreases
from 14.035 to 13.672 at 15 minutes, from 17.283 to 16.604 at 30 minutes,
and from 21.169 to 20.600 at 60 minutes. The congestion-aware model therefore
achieves higher coverage while producing sharper intervals. Together with the
reductions in MAE and RMSE, this indicates that the reweighted training
objective improves the forecasts in the shifted regime rather than merely
increasing the amount of predicted uncertainty.

Figure~\ref{fig:congestion_aware_results} provides a complementary view of
the congestion-aware model by comparing its performance over the complete
test set, normal traffic conditions, and high-congestion periods. The MAE
and RMSE plots show that high congestion remains substantially more difficult
than normal traffic and that the performance gap increases with the forecast
horizon. In particular, point forecasting errors increase considerably at
60 minutes under high congestion, despite the improvements over the original
model reported in Table~\ref{tab:congestion_aware_results}.

The same distinction between traffic regimes is visible in the probabilistic
predictions. As shown in the coverage plot of
Figure~\ref{fig:congestion_aware_results}, coverage remains close to the
nominal 80\% level at 15 minutes but decreases with the forecasting horizon,
especially under high congestion. At the same time, the interval-width plot
shows that prediction intervals become progressively wider for longer
horizons and are substantially wider during congestion than under normal
conditions. This indicates that the model appropriately expresses greater
uncertainty during difficult traffic regimes, although the increase in
interval width is still insufficient to maintain nominal coverage at the
longest horizon.

The improvements under congestion are accompanied by a moderate trade-off
under normal traffic conditions. Normal-condition MAE increases from 2.770
to 2.780 at 15 minutes, from 3.257 to 3.297 at 30 minutes, and from 3.974
to 4.069 at 60 minutes relative to the original Graph-Temporal model.
Emphasizing high-congestion targets during optimization therefore slightly
reduces point-forecasting accuracy in the dominant normal regime. The effect
is limited at short horizons but becomes more visible at 60 minutes.

Overall, the two implemented mitigation strategies address different aspects
of the robustness problem. CQR provides a lightweight post-hoc correction
that brings marginal coverage close to the nominal level without modifying
the median forecasts. Congestion-aware training instead modifies the learned
forecasting function itself and improves both point accuracy and probabilistic
performance within the high-congestion regime, at the cost of a moderate
degradation in normal-condition point accuracy. Neither strategy completely
eliminates the long-horizon calibration gap under severe congestion, but the
results indicate that global post-hoc calibration and regime-aware training
provide complementary forms of mitigation.

\begin{figure}[H]
    \centering

    % Top row: point forecasting accuracy
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]
        {img/congestion_aware_training/mae_distribution_shift}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]
        {img/congestion_aware_training/rmse_distribution_shift}
    \end{subfigure}

    \vspace{0.4cm}

    % Bottom row: probabilistic forecasting
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]
        {img/congestion_aware_training/coverage_distribution_shift}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]
        {img/congestion_aware_training/interval_width_distribution_shift}
    \end{subfigure}

    \caption{Performance of the congestion-aware Graph-Temporal model across
    the evaluated forecasting horizons. The top row reports point forecasting
    accuracy in terms of MAE (left) and RMSE (right), while the bottom row
    reports empirical coverage (left) and prediction interval width (right).
    Results are shown for the overall test set, normal traffic conditions,
    and high-congestion periods.}
    \label{fig:congestion_aware_results}
\end{figure}
...
The mitigation experiments further illustrate different
accuracy-reliability-cost trade-offs. CQR provides an inexpensive post-hoc
mechanism for improving marginal calibration, while congestion-aware training
requires retraining but directly improves forecasting performance in the
high-congestion regime. The latter reduces both point error and interval width
while increasing coverage under congestion, although this comes with a small
loss of accuracy under normal conditions. These results reinforce the need
to evaluate robustness jointly in terms of accuracy, calibration, sharpness,
...
The mitigation experiments also reveal that calibration and robustness can be
addressed at different stages of the forecasting pipeline. CQR effectively
corrects the general undercoverage of the model and brings marginal coverage
close to the nominal 80\% level. Congestion-aware training instead targets
the difficult regime directly during optimization. With $\lambda=1$, it
reduces high-congestion forecasting errors at all evaluated horizons, with
the largest MAE reduction occurring at 60 minutes, while also improving
coverage and producing narrower intervals than the original model under
high congestion. This improvement is accompanied by a moderate degradation
in normal-condition point accuracy, highlighting a trade-off between
performance in the dominant regime and robustness to rare congestion events.

Nevertheless, neither mitigation eliminates the deterioration associated with
longer forecasting horizons. Point forecasting errors increase with the
forecast horizon, particularly under high-congestion conditions; CQR leaves
these errors unchanged by construction, while congestion-aware training
reduces them without eliminating their horizon-dependent increase. Empirical
coverage also becomes more difficult to maintain at longer horizons,
especially under high congestion. At the same time, prediction intervals
remain substantially wider than under normal conditions, reducing their
...
The congestion-aware mitigation also has limitations. The weighting strategy
depends on a binary definition of high congestion and on the choice of the
reweighting parameter $\lambda$. The evaluated $\lambda=1$ configuration
improves performance in the targeted regime but slightly degrades point
accuracy under normal conditions. A broader validation-based analysis of the
weighting strength would therefore be required to characterize this trade-off
more completely.
...
Several extensions could address these limitations. A first extension would
be to investigate the congestion-aware training
strategy more systematically by validating different values of the weighting
parameter $\lambda$. This would allow the trade-off between robustness under
high congestion and performance under normal traffic conditions to be
characterized more precisely. More adaptive weighting schemes could also be
considered, for example by assigning continuous weights based on congestion
severity rather than using a binary normal/high-congestion distinction.
...
Two complementary mitigation strategies were evaluated. Post-hoc calibration
through Conformalized Quantile Regression substantially improves overall
coverage and brings it close to the nominal 80\% level without modifying the
median forecasts or retraining the model. However, residual undercoverage
remains under severe high congestion, particularly at the longest forecasting
horizon.

Congestion-aware training addresses this limitation from a different
perspective by assigning greater importance to high-congestion targets during
optimization. The implemented weighted Pinball Loss improves point forecasting
accuracy under congestion at all evaluated horizons, with the largest gains
at 60 minutes. It also increases empirical coverage while producing narrower
prediction intervals in the shifted regime, indicating that the improvement
does not result from simply increasing predictive uncertainty. These gains
are accompanied by a moderate degradation in point accuracy under normal
conditions, revealing a trade-off between average-regime performance and
robustness to rare congestion events.
